...
fig = plt.figure()
ax = plt.axes(xlim=(xi, xf),ylim=(-5,10))
ax.grid()
line, = ax.plot([], [], lw=1)
line1, = ax.plot([], [], lw=1)
line2, = ax.plot([], [], lw=1)
...
def func(r):
    ...
    indvol=np.append(indvol,initialindvol)
    maxstep=r
    sv=9
    ...
    while math.fabs(cerr)>apps and flag==True :
        ...
        if stepivol>maxvol*0.7:
            stepchange=stepchange*0.5
        if stepivol>maxvol*0.8:
            stepchange=0
        if stepivol>maxvol*0.9:
            flag=False
            #print 'Maximum  voltage limit reached'
        #stepchange=stepchange*math.pow((maxvol*0.9-stepivol)/(maxvol*0.9),0.25)
